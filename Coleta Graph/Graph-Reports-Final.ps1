...
function New-DeviceReport {
    ...
    $compliantPerc = if($totalDevices -gt 0) { [math]::Round(($compliantDevices/$totalDevices)*100,1) } else { 0 }
    $nonCompliantPerc = if($totalDevices -gt 0) { [math]::Round(($nonCompliantDevices/$totalDevices)*100,1) } else { 0 }
    $unknownPerc = if($totalDevices -gt 0) { [math]::Round(($unknownDevices/$totalDevices)*100,1) } else { 0 }
    
    Write-Host "✓ Conformes: $compliantDevices ($compliantPerc%)" -ForegroundColor Green
    Write-Host "✗ Não Conformes: $nonCompliantDevices ($nonCompliantPerc%)" -ForegroundColor Red
    ...
    foreach ($os in $osGroups) {
        $percentage = if($totalDevices -gt 0) { [math]::Round(($os.Count/$totalDevices)*100,1) } else { 0 }
